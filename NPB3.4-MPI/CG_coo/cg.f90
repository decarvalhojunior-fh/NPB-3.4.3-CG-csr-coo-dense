...
!---------------------------------------------------------------------
!  Convert the generated CSR matrix to COO:
!        arow(k) stores the local row for nonzero k
!        colidx(k) stores the local column for nonzero k
!        a(k) stores the matrix value for nonzero k
!---------------------------------------------------------------------
      nzz = rowstr(lastrow-firstrow+2) - 1
...
!---------------------------------------------------------------------
!  q = A.p
!  The partition submatrix-vector multiply using COO storage:
!  use workspace w
!---------------------------------------------------------------------
         do j=1,lastrow-firstrow+1
            w(j) = 0.d0
         enddo
         do k=1,nzz
            w(arow(k)) = w(arow(k)) + a(k)*p(colidx(k))
         enddo
...
!  First, form A.z using COO storage.
!---------------------------------------------------------------------
      do j=1,lastrow-firstrow+1
         w(j) = 0.d0
      enddo
      do k=1,nzz
         w(arow(k)) = w(arow(k)) + a(k)*z(colidx(k))
      enddo
