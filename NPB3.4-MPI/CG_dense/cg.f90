...
!---------------------------------------------------------------------
!  makea stores this process's submatrix as a dense local block in a:
!        a((row-1)*(lastcol-firstcol+1)+col)
!---------------------------------------------------------------------
      nzz = (lastrow-firstrow+1) * (lastcol-firstcol+1)

      write(*,*) firstrow, lastrow, firstcol, lastcol, l2npcols
...
!---------------------------------------------------------------------
         ...
         call mpi_allreduce(norm_temp1, norm_temp2, 2, dp_type,  &
     &                      MPI_SUM, comm_row, ierr)
         norm_temp1(1) = norm_temp2(1)
         norm_temp1(2) = norm_temp2(2)
...
!---------------------------------------------------------------------
     ...
     &                      MPI_SUM, comm_row, ierr)
         ...
         if( me .eq. root .and. &
     &       (it.eq.1 .or. it.eq.niter .or. mod(it,5).eq.0) )then
...
!---------------------------------------------------------------------
      ...
      call mpi_comm_split(comm_solve, proc_row, proc_col,  &
     &                    comm_row, i)
...
!---------------------------------------------------------------------
     ...
     &                 + mod( me,2 )
      ...
      do i = 1, num_procs
         alltoall_send_counts(i) = 0
         alltoall_send_displs(i) = 0
         alltoall_recv_counts(i) = 0
         alltoall_recv_displs(i) = 0
      enddo

      alltoall_send_counts(exch_proc+1) = send_len
      alltoall_send_displs(exch_proc+1) = send_start - 1
      alltoall_recv_counts(exch_proc+1) = exch_recv_length
      alltoall_recv_displs(exch_proc+1) = 0
...
!---------------------------------------------------------------------
      ...
      double precision reduce_send(1), reduce_recv(1)
...
!---------------------------------------------------------------------
!  Sum the partial sums of rho across the process-grid row.
!---------------------------------------------------------------------
      if (timeron) call timer_start(t_rcomm)
      reduce_send(1) = sum
      call mpi_allreduce(reduce_send, reduce_recv, 1, dp_type, MPI_SUM,  &
     &                   comm_row, ierr)
      rho = reduce_recv(1)
...
!---------------------------------------------------------------------
!  q = A.p
!  The dense partition submatrix-vector multiply: use workspace w
!---------------------------------------------------------------------
         do j=1,lastrow-firstrow+1
            sum = 0.d0
            do k=1,ncols
               sum = sum + a((j-1)*ncols+k)*p(k)
            enddo
            w(j) = sum
         enddo

!---------------------------------------------------------------------
!  Sum the partition submatrix-vec A.p's across the process-grid row.
!---------------------------------------------------------------------
         if (timeron) call timer_start(t_rcomm)
         call mpi_allreduce(w, q, lastrow-firstrow+1, dp_type,  &
     &                      MPI_SUM, comm_row, ierr)
         do j=1,lastrow-firstrow+1
            w(j) = q(j)
         enddo
...
!---------------------------------------------------------------------
         if( l2npcols .ne. 0 )then
            if (timeron) call timer_start(t_rcomm)
            call mpi_alltoallv(w, alltoall_send_counts,  &
     &                         alltoall_send_displs, dp_type,  &
     &                         q, alltoall_recv_counts,  &
     &                         alltoall_recv_displs, dp_type,  &
     &                         comm_solve, ierr)
            if (timeron) call timer_stop(t_rcomm)
         else
...
!---------------------------------------------------------------------
         if (timeron) call timer_start(t_rcomm)
         reduce_send(1) = sum
         call mpi_allreduce(reduce_send, reduce_recv, 1, dp_type,  &
     &                      MPI_SUM, comm_row, ierr)
         d = reduce_recv(1)
         if (timeron) call timer_stop(t_rcomm)
...
!  First, form A.z using dense storage.
!---------------------------------------------------------------------
      do j=1,lastrow-firstrow+1
         sum = 0.d0
         do k=1,ncols
            sum = sum + a((j-1)*ncols+k)*z(k)
         enddo
         w(j) = sum
      enddo
...
!---------------------------------------------------------------------
      if (timeron) call timer_start(t_rcomm)
      call mpi_allreduce(w, r, lastrow-firstrow+1, dp_type,  &
     &                   MPI_SUM, comm_row, ierr)
      do j=1,lastrow-firstrow+1
         w(j) = r(j)
      enddo
...
!---------------------------------------------------------------------
!     ... dense local matrix: rows firstrow:lastrow and
!         columns firstcol:lastcol, stored row-major in a.
!---------------------------------------------------------------------
      do k = 1, nrows*ncols
         a(k) = 0.d0
      enddo
      ...
      do nza = 1, nnza
         ...
         i = acol(nza) - firstcol + 1
         if (j.ge.1 .and. j.le.nrows .and.  &
     &       i.ge.1 .and. i.le.ncols) then
            dense_idx = (j-1)*ncols + i
            a(dense_idx) = a(dense_idx) + aelt(nza)
         endif
      enddo
