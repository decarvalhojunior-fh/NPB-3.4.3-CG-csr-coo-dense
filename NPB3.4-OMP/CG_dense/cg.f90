...
!$omp do
         do j=1,lastrow-firstrow+1
            suml = 0.d0
            do k=1,lastcol-firstcol+1
               suml = suml + a(int(j-1,kd)*int(naa,kd)  &
     &              + int(k,kd))*p(k)
            enddo
            q(j) = suml
         enddo
...
!---------------------------------------------------------------------
      ...
      use tinfo
      use cg_data, only : nonzer, rcond, shift, kd
...
!       makea generates a dense matrix with the same values produced by
!       the original sparse matrix generator.
!
...
!       output
!
!       a            r*8         dense matrix values
!       colidx       i           workspace for col indices
!       rowstr       i           workspace for row pointers
!
!       workspace
!
!       iv, arow, acol i
...
!---------------------------------------------------------------------
      ...
      integer          i, iouter, ivelt, j, nzv, nn1
      integer(kz)      k
      integer(kd)      dense_idx
...
!           then scatter the same values into dense storage.
!---------------------------------------------------------------------
      call sparse( v, colidx, rowstr, n, nz, nonzer, arow, acol,  &
     &             aelt, firstrow, lastrow,  &
     &             a, iv(1), iv(nz+1), rcond, shift )
!$omp barrier

!$omp do
      do j = 1, n
         do i = 1, n
            dense_idx = int(j-1,kd)*int(n,kd) + int(i,kd)
            a(dense_idx) = 0.d0
         enddo
      enddo
!$omp end do
...
!$omp do
      do j=1,lastrow-firstrow+1
         do k=rowstr(j),rowstr(j+1)-1
            dense_idx = int(j-1,kd)*int(n,kd)  &
     &                + int(colidx(k)-firstcol+1,kd)
            a(dense_idx) = v(k)
         enddo
      enddo
!$omp end do
