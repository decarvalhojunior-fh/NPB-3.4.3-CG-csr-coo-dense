...
!        rowidx(k) stores the local row for nonzero k
...
!$omp end do
!$omp master
      call sort_coo(rowidx, colidx, a, nzz)
!$omp end master
!$omp barrier
...
!---------------------------------------------------------------------
!  q = A.p
!  Sparse matrix-vector multiply using COO storage.
!---------------------------------------------------------------------
!
...
!$omp do
         do k=1,nzz
!$omp atomic
            q(rowidx(k)) = q(rowidx(k)) + a(k)*p(colidx(k))
         enddo
!$omp end do
...
!$omp do
      do k=1,nzz
!$omp atomic
         r(rowidx(k)) = r(rowidx(k)) + a(k)*z(colidx(k))
      enddo
!$omp end do
...
!---------------------------------------------------------------------
!---------------------------------------------------------------------
      subroutine sort_coo(rowidx, colidx, values, nnz)
!---------------------------------------------------------------------
!---------------------------------------------------------------------
      ...
      integer(kz)        start, finish

      if (nnz .le. 1) return

      do start = nnz/2, 1, -1
         call sift_down_coo(rowidx, colidx, values, start, nnz)
      enddo

      do finish = nnz, 2, -1
         call swap_coo(rowidx, colidx, values, 1_kz, finish)
         call sift_down_coo(rowidx, colidx, values, 1_kz, finish-1)
      enddo
...
!---------------------------------------------------------------------
!---------------------------------------------------------------------
      subroutine sift_down_coo(rowidx, colidx, values, first, last)
!---------------------------------------------------------------------
!---------------------------------------------------------------------
      ...
      integer(kz)        first, last
      integer            rowidx(*), colidx(*)
      double precision   values(*)
      logical            coo_less

      integer(kz)        root, child, swapidx

      root = first

      do while (2*root .le. last)
         child = 2*root
         swapidx = root

         if (coo_less(rowidx, colidx, values, swapidx, child)) then
            swapidx = child
         endif
         if (child+1 .le. last) then
            if (coo_less(rowidx, colidx, values, swapidx, child+1)) then
               swapidx = child+1
            endif
         endif

         if (swapidx .eq. root) then
            return
         else
            call swap_coo(rowidx, colidx, values, root, swapidx)
            root = swapidx
         endif
      enddo
...
!-------end   of sift_down_coo----------------------


!---------------------------------------------------------------------
!---------------------------------------------------------------------
      logical function coo_less(rowidx, colidx, values, left, right)
!---------------------------------------------------------------------
!---------------------------------------------------------------------
      ...
      integer(kz)        left, right
      integer            rowidx(*), colidx(*)
      double precision   values(*)

      if (rowidx(left) .lt. rowidx(right)) then
         coo_less = .true.
      else if (rowidx(left) .gt. rowidx(right)) then
         coo_less = .false.
      else if (colidx(left) .lt. colidx(right)) then
         coo_less = .true.
      else if (colidx(left) .gt. colidx(right)) then
         coo_less = .false.
      else
         coo_less = values(left) .lt. values(right)
      endif
...
!---------------------------------------------------------------------
!---------------------------------------------------------------------
      subroutine swap_coo(rowidx, colidx, values, left, right)
!---------------------------------------------------------------------
!---------------------------------------------------------------------
      ...
      itemp = rowidx(left)
      rowidx(left) = rowidx(right)
      rowidx(right) = itemp

      itemp = colidx(left)
      colidx(left) = colidx(right)
      colidx(right) = itemp
